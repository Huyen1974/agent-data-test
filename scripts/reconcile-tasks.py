...
def directus_api(method, path, data=None, token=None):
    ...
    req = urllib.request.Request(
        f"{DIRECTUS}{path}", data=body, headers=headers, method=method
    )
...
def ad_upsert(payload):
    ...
    req = urllib.request.Request(
        f"{AD}/documents?upsert=true", data=body, headers=headers, method="POST"
    )
    resp = urllib.request.urlopen(req)
...
def main():
    ...
    tasks = directus_api(
        "GET",
        "/items/tasks?limit=-1&fields=id,name,status,priority,assigned_to,description,content_targets,content_rules,content_checklist,content_plan,content_prompt,content_reports,content_verify,content_test&sort=id",
        token=TOKEN,
    )
    ...
    # Get comments
    comments = directus_api(
        "GET",
        "/items/task_comments?limit=-1&fields=id,task_id,tab_scope,agent_type,action,content&sort=id",
        token=TOKEN,
    )
    ...
    log("=== Summary ===")
    log(
        f"Tasks:    {synced_tasks} synced, {missing_tasks} missing"
        + (" (fixed)" if missing_tasks and not DRY_RUN else "")
    )
    log(
        f"Comments: {synced_comments} synced, {missing_comments} missing"
        + (" (fixed)" if missing_comments and not DRY_RUN else "")
    )
    ...
    if missing_tasks == 0 and missing_comments == 0:
        ...
        log(
            f"Would sync {missing_tasks + missing_comments} items (run without --dry-run to fix)"
        )
