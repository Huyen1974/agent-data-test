...
def validate_lark_token(token):
    ...
    try:
        ...
        logger.info(
            f"Validate response status: {response.status_code}, body: {response.text}"
        )
        try:
            data = response.json()
            if data.get("code") == 0:
                ...
                logger.error(
                    f"Lark API error: code={data.get('code')}, msg={data.get('msg')}"
                )
                return False
...
def call_generate_lark_token():
    try:
        ...
        logger.info(
            f"Generate-lark-token response status: {response.status_code}, body: {response.text}"
        )
...
def check_lark_token(request):
    ...
    for attempt in range(MAX_RETRIES):
        ...
        if token is None:
            ...
            logger.warning(
                f"Attempt {attempt + 1}: Failed to retrieve token from Secret Manager"
            )
        ...
        if validate_lark_token(token):
            ...
            return jsonify(
                {"status": "OK", "message": "Token is valid", "token": token}
            )

        log += f"Attempt {attempt + 1}: Token invalid. Calling generate-lark-token...\n"
        logger.info(
            f"Attempt {attempt + 1}: Token invalid. Calling generate-lark-token..."
        )
    ...
    return (
        jsonify(
            {
                "status": "ERROR",
                "message": f"Token invalid after {MAX_RETRIES} attempts.",
            }
        ),
        500,
    )
