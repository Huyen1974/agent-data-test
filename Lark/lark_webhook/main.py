...
GCP_PROJECT_ID = os.environ.get(
    "GCP_PROJECT", "chatgpt-db-project"
)  # Use env var if set
...
try:
    ...
    logging.info(
        f"Firestore and Secret Manager clients initialized successfully for project {FIRESTORE_PROJECT_ID}."
    )
...
def get_secret(project_id, secret_id, version_id="latest") -> str | None:
...
def get_lark_token_from_url(url: str) -> str | None:
    ...
    try:
        ...
        if token:
            logging.info("Successfully fetched Lark token from URL.")
            return token
        else:
            logging.error(
                f"'token' key not found in response JSON from {url}. Response: {response.text}"
            )
            return None
        ...
        if e.response is not None:
            logging.error(
                f"Check Lark Token URL Response Status: {e.response.status_code}, Body: {e.response.text}"
            )
        return None
    except json.JSONDecodeError as e:
        logging.error(
            f"Error decoding JSON response from {url}: {e}. Response: {response.text}",
            exc_info=True,
        )
        return None
    except Exception as e:  # Catch any other unexpected errors
        logging.error(
            f"Unexpected error fetching Lark token from {url}: {e}", exc_info=True
        )
        return None
...
def send_lark_message(chat_id: str, content: str, token: str) -> bool:
    ...
    try:
        response = requests.post(
            LARK_SEND_MESSAGE_URL, headers=headers, data=payload, timeout=15
        )
        ...
        if response_data.get("code") == 0:
            ...
            logging.error(
                f"Failed to send Lark message. Lark Code: {lark_code}, Msg: {lark_msg}"
            )
            return False
    except requests.exceptions.Timeout:
        logging.error(
            f"Timeout error sending Lark message to {chat_id}.", exc_info=True
        )
        return False
    except requests.exceptions.RequestException as e:
        ...
        if e.response is not None:
            logging.error(
                f"Lark send API Response Status: {e.response.status_code}, Body: {e.response.text}"
            )
        return False
    except json.JSONDecodeError as e:
        logging.error(
            f"Error decoding JSON response from Lark send API: {e}. Response: {response.text}",
            exc_info=True,
        )
        return False
...
def call_openai_api(
    api_key: str, system_prompt: str, messages: list
    ...
    try:
        ...
        logging.error(
            f"Error decoding JSON response from OpenAI API: {e}. Response: {response.text}",
            exc_info=True,
        )
...
def function_comment(parsed_command: str):
    ...
    print(
        f"function_comment processed command: {parsed_command}"
    )  # Replaced logging.info
...
def handle_command(parsed_command: str):
    ...
    print(
        f"Calling handle_command with command: {parsed_command}"
    )  # Replaced logging.info
...
def update_conversation(conversation_id: str, updates: dict[str, Any]) -> bool:
    ...
    try:
        ...
        logging.info(
            f"Conversation {conversation_id} updated successfully in Firestore."
        )
        logging.debug(f"Firestore update data for {conversation_id}: {updates}")
        return True
    except Exception as e:
        logging.error(
            f"Error updating Firestore document {conversation_id}: {e}", exc_info=True
        )
        return False


def create_conversation(data: dict[str, Any]) -> str | None:
    ...
    try:
        ...
        doc_ref.set(data)
        logging.info(
            f"Conversation {conversation_id} created successfully in Firestore."
        )
...
def get_pending_conversation(chat_id: str) -> firestore.DocumentSnapshot | None:
    ...
    if not db:
        logging.error(
            "Firestore client not initialized. Cannot get pending conversation."
        )
        return None
    if not chat_id:
    ...
    try:
        logging.debug(
            f"Querying Firestore for pending conversation for chat_id: {chat_id}"
        )
        query = (
        ...
        logging.error(
            f"Error querying Firestore for pending conversation: {e}", exc_info=True
        )
        return None
...
def handle_challenge(request_json: dict[str, Any]) -> Any:
    ...
    if not challenge_code:
        ...
        return (
            json.dumps(
                {"status": "ERROR", "message": "Missing challenge code in request"}
            ),
            400,
            {"Content-Type": "application/json"},
        )

    logging.info("Received Lark challenge request. Responding with challenge code.")
...
def parse_lark_event(request_json: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        ...
        is_v2 = schema == "2.0" or (
            header and header.get("event_type") == "im.message.receive_v1"
        )
        ...
        if is_v2:
            ...
            if extracted_data["message_type"] == "text":
                ...
                try:
                    ...
                    logging.debug(
                        f"V2 Text content extracted: {extracted_data['user_message'][:100]}..."
                    )
                except json.JSONDecodeError:
                    logging.warning(
                        f"Could not decode V2 message content JSON: {content_str}"
                    )
            elif extracted_data["message_type"]:
                # Log unsupported types but continue parsing other fields
                logging.info(
                    f"Received unsupported message_type (V2): {extracted_data['message_type']} from chat {extracted_data['chat_id']}"
                )
            else:
        ...
        # --- Fallback/Assume V1 Structure ---
            ...
            extracted_data["chat_id"] = event.get(
                "chat_id"
            )  # V1 might have chat_id directly in event
            ...
            if extracted_data["message_type"] == "text":
                ...
                try:
                    ...
                    extracted_data["user_message"] = content.get("text", "").strip()
                    logging.debug(
                        f"V1 Text content extracted (from JSON): {extracted_data['user_message'][:100]}..."
                    )
                except json.JSONDecodeError:
                    ...
                    if isinstance(content_str, str):
                        ...
                        logging.debug(
                            f"V1 message content treated as raw string: {extracted_data['user_message'][:100]}..."
                        )
                    else:
                        logging.warning(
                            f"Could not decode V1 message content: {content_str}"
                        )
                except (
                    TypeError
                ):  # Handle cases where content might not be string/bytes
                    logging.warning(
                        f"Invalid type for V1 message content: {type(content_str)}"
                    )

            elif extracted_data["message_type"]:
                logging.info(
                    f"Received unsupported message_type (V1): {extracted_data['message_type']} from chat {extracted_data['chat_id']}"
                )
            else:
            ...
            logging.error(
                f"Could not determine event schema (V1/V2). Keys: {list(request_json.keys())}"
            )
            return None
        ...
        if not extracted_data["message_id"]:
            logging.warning(
                f"Failed to extract message_id from event for chat {extracted_data['chat_id']}. Proceeding without it."
            )

        # Filter by Target Chat ID
        if extracted_data["chat_id"] != TARGET_LARK_CHAT_ID:
            logging.info(
                f"Ignoring message from non-target chat ID: {extracted_data['chat_id']}"
            )
            return None
        ...
        if extracted_data["message_type"] != "text":
            logging.info(
                f"Ignoring non-text message. Type: {extracted_data['message_type']}"
            )
            return None

        # Filter by Empty Content
        if not extracted_data["user_message"]:
            logging.info("Ignoring text message with empty content.")
            return None

        logging.info(
            f"Successfully parsed relevant Lark text event: chat_id={extracted_data['chat_id']}, msg_id={extracted_data['message_id']}"
        )
...
def handle_message_event(parsed_event: dict[str, Any]) -> tuple[Any, int]:
    ...
    if not chat_id or not user_message:
        logging.error(
            "handle_message_event called with missing chat_id or user_message."
        )
    ...
    if lark_token:
        ...
        print(
            f"Debug: Successfully retrieved Lark token: {lark_token[:5]}...{lark_token[-5:]}"
        )
    else:
        ...
        logging.error(
            "Failed to retrieve Lark token from check_lark_token URL. Aborting message handling."
        )
        # Cannot notify user without token
        return {
            "status": "ERROR",
            "message": "Server configuration error (Could not get Lark Token from URL)",
        }, 500
    ...
    if not openai_api_key:
        ...
        send_lark_message_success = send_lark_message(
            chat_id,
            "Lỗi: Không thể truy cập khóa API OpenAI. Vui lòng liên hệ quản trị viên.",
            lark_token,
        )
        print(
            f"Debug: Lark message sent (reporting OpenAI key error): {send_lark_message_success}"
        )
        return {
            "status": "ERROR",
            "message": "Server configuration error (OpenAI Key)",
        }, 500
    ...
    if pending_conv_snapshot:
        ...
        if not conv_data:
            logging.error(
                f"Failed to read data for pending conversation {conv_id}. Aborting."
            )
            send_lark_message_success = send_lark_message(
                chat_id,
                "Lỗi: Không thể đọc dữ liệu cuộc hội thoại đang chờ.",
                lark_token,
            )
            print(
                f"Lark message sent (reporting Firestore read error): {send_lark_message_success} for conversation {conv_id}"
            )
            print(
                f"Debug: Lark message sent (reporting Firestore read error): {send_lark_message_success} for conversation {conv_id}"
            )
            update_success = update_conversation(
                conv_id,
                {
                    "labels.Status": "Failed",
                    "bot_response": "Error reading conversation data",
                },
            )
            print(
                f"Firestore document updated (marking read error): {update_success} for conversation {conv_id}"
            )
            print(
                f"Debug: Firestore document updated (marking read error): {update_success} for conversation {conv_id}"
            )
        ...
        if (
            ...
            logging.warning(
                f"Duplicate user message detected for {conv_id}. History[-1]: {history[-1]}. Current message: {user_message}. Ignoring duplicate and acknowledging request."
            )
            return "OK", 200  # Acknowledge receipt, state hasn't changed meaningfully

        # Check if the user is confirming
        is_confirming = any(
            keyword in user_message.lower() for keyword in CONFIRMATION_KEYWORDS
        )
        ...
        if is_confirming:
            ...
            if not confirmed_command:
                logging.error(
                    f"Confirmation received for {conv_id}, but couldn't find last assistant message in history: {history}"
                )
                send_lark_message_success = send_lark_message(
                    chat_id,
                    "Lỗi: Không tìm thấy phản hồi trước đó để xác nhận.",
                    lark_token,
                )
                print(
                    f"Lark message sent (reporting confirmation error): {send_lark_message_success} for conversation {conv_id}"
                )
                print(
                    f"Debug: Lark message sent (reporting confirmation error): {send_lark_message_success} for conversation {conv_id}"
                )
                update_success = update_conversation(
                    conv_id,
                    {
                ...
                print(
                    f"Firestore document updated (marking confirmation error): {update_success} for conversation {conv_id}"
                )
                print(
                    f"Debug: Firestore document updated (marking confirmation error): {update_success} for conversation {conv_id}"
                )
                return {
                    "status": "ERROR",
                    "message": "Logic error: Could not find previous response",
                }, 500
            ...
            update_successful = update_conversation(
                conv_id,
                {
            ...
            print(
                f"Firestore document updated (status Completed): {update_successful} for conversation {conv_id}"
            )
            print(
                f"Debug: Firestore document updated (status Completed): {update_successful} for conversation {conv_id}"
            )

            if update_successful:
                handle_command(confirmed_command)
                send_lark_message_success = send_lark_message(
                    chat_id, "Đã xác nhận. Đang xử lý yêu cầu của bạn...", lark_token
                )
                print(
                    f"Lark message sent (confirmation success): {send_lark_message_success} for conversation {conv_id}"
                )
                print(
                    f"Debug: Lark message sent (confirmation success): {send_lark_message_success} for conversation {conv_id}"
                )
            else:
                send_lark_message_success = send_lark_message(
                    chat_id,
                    "Lỗi: Không thể cập nhật trạng thái cuộc hội thoại sau khi xác nhận.",
                    lark_token,
                )
                print(
                    f"Lark message sent (reporting update failure): {send_lark_message_success} for conversation {conv_id}"
                )
                print(
                    f"Debug: Lark message sent (reporting update failure): {send_lark_message_success} for conversation {conv_id}"
                )
                return {
                    "status": "ERROR",
                    "message": "Firestore update failed after confirmation",
                }, 500
        else:
            # --- User provided more info, ask GPT again ---
            print(
                f"User provided additional information for {conv_id}. Request count: {request_count}"
            )
            if request_count >= MAX_CLARIFICATION_ATTEMPTS:
                logging.warning(
                    f"Max clarification attempts ({MAX_CLARIFICATION_ATTEMPTS}) reached for {conv_id}."
                )
                fail_message = (
                    "Đã vượt quá số lần hỏi lại tối đa. Không thể xử lý yêu cầu."
                )
                ...
                print(
                    f"Firestore document updated (max attempts): {update_success} for conversation {conv_id}"
                )
                print(
                    f"Debug: Firestore document updated (max attempts): {update_success} for conversation {conv_id}"
                )
                send_lark_message_success = send_lark_message(
                    chat_id, fail_message, lark_token
                )
                print(
                    f"Lark message sent (max attempts): {send_lark_message_success} for conversation {conv_id}"
                )
                print(
                    f"Debug: Lark message sent (max attempts): {send_lark_message_success} for conversation {conv_id}"
                )
            else:
                bot_response, error = call_openai_api(
                    openai_api_key, OPENAI_SYSTEM_PROMPT, history
                )
                # Added Logging for OpenAI API Call
                if error:
                    print(f"OpenAI API call failed for conversation {conv_id}: {error}")
                    print(
                        f"Debug: OpenAI API call failed for conversation {conv_id}: {error}"
                    )
                    send_lark_message_success = send_lark_message(
                        chat_id, f"Lỗi khi xử lý thông tin bổ sung: {error}", lark_token
                    )
                    print(
                        f"Lark message sent (reporting OpenAI error): {send_lark_message_success} for conversation {conv_id}"
                    )
                    print(
                        f"Debug: Lark message sent (reporting OpenAI error): {send_lark_message_success} for conversation {conv_id}"
                    )
                    # Firestore Update: Update history and user_message even if AI call fails
                    update_success = update_conversation(
                        conv_id, {"history": history, "user_message": user_message}
                    )
                    print(
                        f"Firestore document updated (after OpenAI failure): {update_success} for conversation {conv_id}"
                    )
                    print(
                        f"Debug: Firestore document updated (after OpenAI failure): {update_success} for conversation {conv_id}"
                    )
                elif bot_response:
                    print(f"OpenAI API called successfully for conversation {conv_id}.")
                    print(
                        f"Debug: OpenAI API called successfully for conversation {conv_id}."
                    )
                    history.append({"role": "assistant", "content": bot_response})
                    message_sent = send_lark_message(chat_id, bot_response, lark_token)
                    # Added Logging for Lark Message Send
                    print(
                        f"Lark message sent (clarification response): {message_sent} for conversation {conv_id}"
                    )
                    print(
                        f"Debug: Lark message sent (clarification response): {message_sent} for conversation {conv_id}"
                    )
                    if message_sent:
                        # Firestore Update: Add top-level user_message
                        update_success = update_conversation(
                        ...
                        print(
                            f"Firestore document updated (clarification pending): {update_success} for conversation {conv_id}"
                        )
                        print(
                            f"Debug: Firestore document updated (clarification pending): {update_success} for conversation {conv_id}"
                        )
                    else:
                        logging.error(
                            f"Failed to send Lark message for {conv_id}, Firestore update skipped for this turn."
                        )
                        ...
                        print(
                            f"Firestore document updated (after Lark send failure): {update_success} for conversation {conv_id}"
                        )
                        print(
                            f"Debug: Firestore document updated (after Lark send failure): {update_success} for conversation {conv_id}"
                        )
    else:
        # --- Handle New Request ---
        ...
        bot_response, error = call_openai_api(
            openai_api_key, OPENAI_SYSTEM_PROMPT, initial_history
        )
        # Added Logging for OpenAI API Call
        if error:
            ...
            send_lark_message_success = send_lark_message(
                chat_id, f"Lỗi khi xử lý yêu cầu ban đầu: {error}", lark_token
            )
            print(
                f"Lark message sent (reporting initial OpenAI error): {send_lark_message_success}"
            )
            print(
                f"Debug: Lark message sent (reporting initial OpenAI error): {send_lark_message_success}"
            )
            ...
            if conv_id:
                ...
                # Added Logging for Lark Message Send
                print(
                    f"Lark message sent (initial response): {message_sent} for conversation {conv_id}"
                )
                print(
                    f"Debug: Lark message sent (initial response): {message_sent} for conversation {conv_id}"
                )
                if not message_sent:
                    logging.error(
                        f"Created conversation {conv_id} but failed to send initial Lark message."
                    )
                    ...
                    print(
                        f"Firestore document updated (marking initial send failed): {update_success} for conversation {conv_id}"
                    )
                    print(
                        f"Debug: Firestore document updated (marking initial send failed): {update_success} for conversation {conv_id}"
                    )
                ...
                send_lark_message_success = send_lark_message(
                    chat_id,
                    "Lỗi: Không thể lưu trữ cuộc hội thoại ban đầu.",
                    lark_token,
                )
                print(
                    f"Lark message sent (reporting create failure): {send_lark_message_success}"
                )
                print(
                    f"Debug: Lark message sent (reporting create failure): {send_lark_message_success}"
                )
...
def lark_webhook(request):
    ...
    logging.info(
        f"lark_webhook invoked. Method: {request.method}, Content-Type: {request.content_type}, Path: {request.path}"
    )

    # Check if clients initialized (critical)
    if not db or not secret_manager_client:
        logging.critical(
            "Firestore or Secret Manager client not available. Function cannot proceed."
        )
        # Use standard tuple return format
        return (
            json.dumps(
                {
                    "status": "ERROR",
                    "message": "Internal Server Error: Service dependencies not initialized.",
                }
            ),
            500,
            {"Content-Type": "application/json"},
        ...
        if request.method == "GET":
            ...
            return (
                json.dumps(
                    {
                        "status": "OK",
                        "message": "Service is running. Use POST for webhook.",
                    }
                ),
                200,
                {"Content-Type": "application/json"},
    ...
    if not request.is_json:
        logging.warning(
            f"Received POST request but Content-Type is not JSON: {request.content_type}"
        )
        # Use standard tuple return format
        return (
            json.dumps(
    ...
    try:
        if "challenge" in request_json:
            ...
            if parsed_event:
                ...
                logging.info(
                    "Request parsed but determined irrelevant or failed parsing, acknowledging request."
                )
                ...
                return (
                    json.dumps(
                        {
                            "status": "OK",
                            "message": "Event received but not processed (e.g., wrong chat, non-text, or parse error)",
                        }
                    ),
                    200,
                    {"Content-Type": "application/json"},
        ...
        logging.error(
            f"Unhandled exception in main webhook handler: {e}", exc_info=True
        )
        ...
        try:
            ...
            chat_id_notify = (
                request_json.get("event", {}).get("message", {}).get("chat_id")
            )

            if chat_id_notify and lark_token_notify:
                logging.info(
                    f"Attempting to send final error notification to chat {chat_id_notify}"
                )
                send_lark_message(
                    chat_id_notify,
                    "Đã xảy ra lỗi máy chủ nội bộ không mong muốn khi xử lý yêu cầu của bạn.",
                    lark_token_notify,
                )
            elif chat_id_notify:
                logging.error(
                    "Could not get Lark token to send final error notification."
                )
            else:
                logging.error(
                    "Cannot send final error notification to Lark (missing chat_id or token)."
                )
        except Exception as notify_e:
            logging.error(
                f"Exception occurred while trying to send final error notification to Lark: {notify_e}",
                exc_info=True,
            )
